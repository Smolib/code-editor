...
function App() {
  ...
  return (
    <>
      <header className="header"><h1 className="header__title">Здесь мы научим вас магии!</h1></header>
      <main className="main">
        <div className="main__wrapper">
          <div className="main__wrapper-task">
            ...
            <div className="main__wrapper-task-text">
              <h2 className="main__title-task">Выполните сложное первое задание</h2>
              <p className="main__description-task">Ваша задача - написать такой код, который сможет победить рандом на сервере и выдать успешный результат.</p>
              <p className="main__description-task">Выбирайте язык, пишите код, нажимайте кнопку Run и ожидайте.</p>
              <p className="main__description-task">И да пребудет с вами удача!</p>
            </div>
          </div>
          <Editor selectedLanguage={selectedLanguage} ref={editorRef}></Editor>
        </div>
        <p className={`main__sucess-response main__success-response_color_${codeCheckTheme.color}`}>
          {codeCheckTheme.text}
        </p>
